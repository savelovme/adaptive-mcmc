...
def pth_moment_rmse(x, y, p=2.0):
    ...
    rmse = jnp.linalg.vector_norm(pth_moment_x - pth_moment_y)
...
def gaussian_kernel(x, y, gamma):
    ...
    eucl_dist2 = ((x[:, None, :] - y[None, :, :]) ** 2).sum(axis=-1)
...
@jit
def mmd_heuristic(x, y):
    """
    Compute the (biased) MMD estimator between two sets of samples.
    ...
    x : jnp.ndarray
        Array of shape (n, d).
    y : jnp.ndarray
        Array of shape (m, d) with reference samples.
    ...
    # Set bandwidth according to median heuristic
    gamma = 4.0 / jnp.median( ((y[:, None, :] - y[None, :, :]) ** 2).sum(-1) )
    ...
    # Compute each term in the MMD formula
    mmd2 = jnp.sum(Kxx) / n**2 + jnp.sum(Kyy) / m**2- 2 * jnp.sum(Kxy) / (n * m)
